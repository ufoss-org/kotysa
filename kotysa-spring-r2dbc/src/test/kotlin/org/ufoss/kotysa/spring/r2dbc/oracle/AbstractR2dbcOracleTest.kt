...
abstract class AbstractR2dbcOracleTest<T : Repository> : ReactorRepositoryTest<T, ReactorTransaction>,
    ...
    protected abstract fun instantiateRepository(
        sqlClient: OracleReactorSqlClient,
        coSqlClient: OracleCoroutinesSqlClient
    ): T

    override val repository: T by lazy {
        val databaseClient = context.getBean<DatabaseClient>()
        instantiateRepository(databaseClient.sqlClient(oracleTables), databaseClient.coSqlClient(oracleTables))
    }
