...
abstract class MutinySelectIntAsIdentitiesRepository<T : IntAsIdentities>(
    private val sqlClient: MutinySqlClient,
    private val table: T,
) : Repository {

    internal lateinit var generatedIntAsIdentitiesWithNullable: IntEntityAsIdentity
    internal lateinit var generatedIntAsIdentitiesWithoutNullable: IntEntityAsIdentity
    ...
    fun insert(): Uni<IntEntityAsIdentity> =
        (sqlClient insertAndReturn intAsIdentityWithNullable)
            .invoke { inserted -> run { generatedIntAsIdentitiesWithNullable = inserted } }
            .chain { -> (sqlClient insertAndReturn intAsIdentityWithoutNullable) }
            .invoke { inserted -> run { generatedIntAsIdentitiesWithoutNullable = inserted } }
