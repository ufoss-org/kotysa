...
class ByteArrayRepositoryH2Select(private val sqlClient: R2dbcSqlClient) : Repository {
    ...
    private suspend fun createTables() {
        sqlClient createTable H2ByteArrayBinarys
    }
    ...
    private suspend fun deleteAll() = sqlClient deleteAllFrom H2ByteArrayBinarys

    fun selectAllByByteArrayNotNull(byteArray: ByteArray) =
            (sqlClient selectFrom H2ByteArrayBinarys
                    where H2ByteArrayBinarys.byteArrayNotNull eq byteArray
                    ).fetchAll()

    fun selectAllByByteArrayNotNullNotEq(byteArray: ByteArray) =
            (sqlClient selectFrom H2ByteArrayBinarys
                    where H2ByteArrayBinarys.byteArrayNotNull notEq byteArray
                    ).fetchAll()

    fun selectAllByByteArrayNotNullIn(values: Sequence<ByteArray>) =
            (sqlClient selectFrom H2ByteArrayBinarys
                    where H2ByteArrayBinarys.byteArrayNotNull `in` values
                    ).fetchAll()

    fun selectAllByByteArrayNullable(byteArray: ByteArray?) =
            (sqlClient selectFrom H2ByteArrayBinarys
                    where H2ByteArrayBinarys.byteArrayNullable eq byteArray
                    ).fetchAll()

    fun selectAllByByteArrayNullableNotEq(byteArray: ByteArray?) =
            (sqlClient selectFrom H2ByteArrayBinarys
                    where H2ByteArrayBinarys.byteArrayNullable notEq byteArray
                    ).fetchAll()
}
