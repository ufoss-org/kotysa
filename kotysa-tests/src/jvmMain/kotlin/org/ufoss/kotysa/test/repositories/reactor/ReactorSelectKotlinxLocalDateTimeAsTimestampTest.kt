...
interface ReactorSelectKotlinxLocalDateTimeAsTimestampTest<T : KotlinxLocalDateTimeAsTimestamps,
        U : ReactorSelectKotlinxLocalDateTimeAsTimestampRepository<T>, V : Transaction> : ReactorRepositoryTest<U, V> {
    ...
    fun `Verify selectAllByLocalDateTimeNullableNotEq finds no results`() {
        assertThat(
            repository.selectAllByLocalDateTimeNullableNotEq(null).toIterable()
        )
            .hasSize(1)
            .containsExactlyInAnyOrder(kotlinxLocalDateTimeAsTimestampWithNullable)
    }
