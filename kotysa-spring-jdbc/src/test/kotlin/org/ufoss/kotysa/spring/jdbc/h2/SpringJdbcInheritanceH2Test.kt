...
class InheritanceH2Repository(client: JdbcOperations) : Repository {
    ...
    fun <T : Parent<U>, U : Entity<String>> selectById(table: T, id: String) =
            (sqlClient selectFrom table
                    where table.id eq id
                    ).fetchOne()

    fun <T : Parent<U>, U : Nameable> selectFirstByName(table: T, name: String) =
            (sqlClient selectFrom table
                    where table.name eq name
                    ).fetchFirst()

    fun <T : Parent<U>, U : Entity<String>> deleteById(table: T, id: String) =
